<x-modal-component :id="$modalID" :title="$modalTitle">
    ...
    <div class="fv-row mb-8">
        ...
        <div class="d-flex w-full gap-5">
            <x-input-radio id="is_activeYes" name="is_active" value="1" model="form.is_active" label="Yes" />
            <x-input-radio id="is_activeNo" name="is_active" value="0" model="form.is_active" label="No" />
        </div>
    ...
    @slot('footer')
        ...
        <x-button type="submit" class="btn btn-primary"
            wire:click="{{ $form->user ? 'update' : 'create' }}">{{ $form->user ? 'Update' : 'Add' }}</x-button>
    @endslot
</x-modal-component>
