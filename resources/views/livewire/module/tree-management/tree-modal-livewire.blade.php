<x-modal-component :id="$modalID" :title="$modalTitle">
    ...
    @if ($form->tree)
        <div class="fv-row mb-8">
            <x-input-label for="tree_tag" class="mb-2" :value="__('messages.tree_tag')" />
            <x-input-text id="tree_tag" wire:model="form.tree_tag" disabled="true" />
        </div>
    @endif

    <div class="fv-row mb-8">
        <x-input-label for="species" class="required mb-4" :value="__('messages.species')" />
    ...
    <div class="fv-row mb-8">
        <div class="d-flex w-100 gap-5">
            <div class="flex-fill">
                <x-input-label for="height" class="required mb-4" :value="__('messages.initial_height').'(m)'" />
                <x-input-text id="height" type="number" placeholder="Height(m)" wire:model="form.height" />
            </div>

            <div class="flex-fill">
                <x-input-label for="diameter" class="required mb-4" :value="__('messages.initial_diameter').'(m)'" />
    ...
    <div class="fv-row mb-8">
        <x-input-label for="flowering_period" class="required mb-2" :value="__('messages.initial_flowering_period')" />
    ...
    @slot('footer')
        ...
        <x-button type="submit" class="btn btn-primary"
            wire:click="{{ $form->tree ? 'update' : 'create' }}">{{ $form->tree ? __('messages.update') : __('messages.add') }}</x-button>
    @endslot
</x-modal-component>
