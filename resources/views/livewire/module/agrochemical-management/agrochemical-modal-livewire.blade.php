<x-modal-component :id="$modalID" :title="$modalTitle">
    ...
    <div class="fv-row mb-8">
        <div class="d-flex w-100 gap-5">
            <div class="flex-fill">
                ...
                <x-input-text id="quantity_per_unit" type="number" min="0" placeholder="Quantity per unit (kg/litre)" wire:model="form.quantity_per_unit" />
            </div>

            <div class="flex-fill">
                <x-input-label for="price" class="required mb-4" :value="__('Price(RM)')" />
                <x-input-text id="price" type="number" min="0" placeholder="Price(RM)" wire:model="form.price" />
            </div>
        </div>
    ...
    <div class="fv-row mb-8">
        <x-input-label for="type" class="required mb-4" :value="__('Type')" />
        <x-input-select id="type" placeholder="Select Type" wire:model="form.type" :options="$typeOptions" />
    ...
    @slot('footer')
        ...
        <x-button type="submit" class="btn btn-primary" wire:click="{{ $form->agrochemical ? 'update' : 'create' }}">{{ $form->agrochemical ? 'Update' : 'Add' }}</x-button>
    @endslot
</x-modal-component>
